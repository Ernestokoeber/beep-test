...
BT.tactics = (function() {
  ...
  function cloneStep(step) { return { id: stepId(), duration: step.duration, elements: copy(step.elements).map(element => Object.assign(element, { id: id('el_') })) }; }
  ...
  function render(target) {
    ...
    function addElement(element) { if (cur().elements.length >= 41 && !['offense', 'defense', 'ball'].includes(element.type)) { if (toast) toast('Maximal 30 Zeichenobjekte pro Schritt.'); return; } cur().elements.push(element); select(element); persist(); refresh(); }
  ...
  async function exportPdf(board) { try { const JsPDF = await loadPdf(), doc = new JsPDF({ unit: 'pt', format: 'a4', orientation: 'landscape' }); board.steps.forEach((step, index) => { if (index) doc.addPage(); doc.setFontSize(20); doc.text(board.title || 'Taktikboard', 40, 38); doc.setFontSize(11); doc.text(board.description || 'CourtHub Teamtaktik', 40, 57); doc.text('Schritt ' + (index + 1) + ' von ' + board.steps.length + ' · ' + step.duration + ' s', 40, 74); drawPdfCourt(doc, step, 135, 90, 550, 517); doc.setFontSize(10); doc.text('Legende: O Angriff · X Verteidigung · ● Ball · Grün Laufweg · Orange Pass · gestrichelt Dribbling', 40, 555); }); const blob = doc.output('blob'); download((board.title || 'taktik').replace(/[^a-z0-9]+/gi, '_').toLowerCase() + '.pdf', blob); if (toast) toast('Taktik-PDF erstellt.'); } catch (error) { if (toast) toast('PDF-Export fehlgeschlagen: ' + error.message); } }
  function drawPdfCourt(doc, step, x, y, width, height) { const sx = width / 500, sy = height / 470, px = value => x + value * sx, py = value => y + value * sy; doc.setDrawColor(122, 74, 26); doc.rect(x, y, width, height); doc.rect(px(160), py(10), 180 * sx, 190 * sy); doc.circle(px(250), py(200), 60 * sx); elements(step).forEach(element => { if (element.type === 'zone') { doc.setDrawColor(46, 110, 170); if (element.shape === 'circle') doc.ellipse(px(element.x), py(element.y), element.width * sx / 2, element.height * sy / 2); else doc.rect(px(element.x - element.width / 2), py(element.y - element.height / 2), element.width * sx, element.height * sy); } else if (element.type === 'arrow') { doc.setDrawColor(element.kind === 'pass' ? 232 : 0, element.kind === 'pass' ? 161 : 75, element.kind === 'pass' ? 77 : 43); if (element.kind === 'pass' || element.kind === 'dribble') doc.setLineDashPattern([5, 4], 0); doc.line(px(element.x), py(element.y), px(element.x2), py(element.y2)); doc.setLineDashPattern([], 0); } else if (['offense', 'defense'].includes(element.type)) { doc.setFillColor(element.type === 'offense' ? 232 : 46, element.type === 'offense' ? 161 : 110, element.type === 'offense' ? 77 : 170); if (element.type === 'offense') doc.circle(px(element.x), py(element.y), 13 * sx, 'FD'); else doc.rect(px(element.x) - 13 * sx, py(element.y) - 13 * sy, 26 * sx, 26 * sy, 'FD'); doc.setTextColor(255, 255, 255); doc.setFontSize(8); doc.text(element.role, px(element.x), py(element.y) + 3, { align: 'center' }); doc.setTextColor(0, 0, 0); } else if (element.type === 'ball') { doc.setFillColor(230, 126, 34); doc.circle(px(element.x), py(element.y), 7 * sx, 'FD'); } else if (element.type === 'cone') { doc.setFillColor(232, 161, 77); doc.triangle(px(element.x), py(element.y - 10), px(element.x - 9), py(element.y + 9), px(element.x + 9), py(element.y + 9), 'F'); } else if (element.type === 'label') { doc.setFontSize(10); doc.text(element.text, px(element.x), py(element.y), { align: 'center' }); } }); }
  ...
  return { render, renderPlayer, normalizeBoard, templates, interpolateStep };
})();
