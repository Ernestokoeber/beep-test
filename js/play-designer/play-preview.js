...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .chp-preview{width:100%;min-height:100dvh;margin:0;padding-bottom:var(--courthub-safe-bottom,env(safe-area-inset-bottom,0px));background:#fff;color:#282321}
    .chp-head{position:sticky;z-index:3;top:0;display:flex;align-items:center;justify-content:space-between;gap:1rem;height:calc(3.5rem + var(--courthub-safe-top,env(safe-area-inset-top,0px)));padding:var(--courthub-safe-top,env(safe-area-inset-top,0px)) max(1.4rem,var(--courthub-safe-right,env(safe-area-inset-right,0px))) 0 max(1.4rem,var(--courthub-safe-left,env(safe-area-inset-left,0px)));border-bottom:1px solid #eceeef;background:rgba(255,255,255,.96);backdrop-filter:blur(.6rem)}
    ...
    @media(max-width:720px){.chp-head{padding-top:var(--courthub-safe-top,env(safe-area-inset-top,0px));padding-right:max(.55rem,var(--courthub-safe-right,env(safe-area-inset-right,0px)));padding-left:max(.55rem,var(--courthub-safe-left,env(safe-area-inset-left,0px)))}.chp-head h1{max-width:10rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.chp-actions .chq-btn:not(.primary){padding:.35rem}.chp-intro,.chp-phases{width:min(54rem,calc(100% - 2rem - var(--courthub-safe-left,0px) - var(--courthub-safe-right,0px)))}.chp-phases{gap:1.2rem;padding-top:1.5rem}.chp-phase{grid-template-columns:1fr;gap:.75rem}.chp-phase-diagram{width:100%}.chp-actions .primary{right:max(.65rem,var(--courthub-safe-right,env(safe-area-inset-right,0px)));bottom:calc(.65rem + var(--courthub-safe-bottom,env(safe-area-inset-bottom,0px)));min-height:44px}}
  `;
